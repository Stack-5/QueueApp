...
const AdminSettingsScreen = () => {
  ...
  return (
    ...
    <Text style={{textAlign: "center", fontSize: wp(10), color: "#FFBF00", fontFamily: "lexendmedium"}}>{userInfo?.role}</Text>
    <Text style={{textAlign:"center", fontSize: wp(7), color: "#0077B6", marginTop: hp(2), fontFamily: "lexendregular", marginBottom: hp(2)}}>{userInfo?.name}</Text>

    <NeuQueueSettingsButton iconName="account-cash" label="Manage employees" onPress={() => router.push("/admin/manage-cashier")}/>
    <NeuQueueSettingsButton iconName="account-details" label="View logs" onPress={() => {}}/>
    <NeuQueueSettingsButton iconName="account-alert" label="Review pending accounts" onPress={() => {router.push("/admin/manage-pending")}}/>
